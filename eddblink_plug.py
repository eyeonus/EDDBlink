...
class ImportPlugin(plugins.ImportPluginBase):
    ...
    def importCommodities(self):
        ...
        with open(str(self.dataPath / self.commoditiesPath), "rU") as fh:
            ...
            if not any(c.get('name', None) == '" not in fh:' for c in commodities):
    ...
    def run(self):
        ...
        # Adding this column allows the listener to track 
        # the mobile stations when they move between systems.
        if (tmpFile.find("type_id INTEGER DEFAULT 0 NOT NULL,") == -1) and not firstRun:
            self.execute("ALTER TABLE Station ADD type_id INTEGER DEFAULT 0 NOT NULL")
        tmpFile = tmpFile.replace("CHECK (planetary  IN ('?', 'Y', 'N')),\n\n   UNIQUE (station_id),","CHECK (planetary  IN ('?', 'Y', 'N')),\n   type_id INTEGER DEFAULT 0 NOT NULL,\n\n   UNIQUE (station_id),")
